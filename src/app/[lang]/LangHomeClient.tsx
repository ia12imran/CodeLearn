...
import { getTopics, getTotalLessons, getExtraQuiz, getPracticeExercises, getFirstLessonHref, LANG_META, Language } from "@/data/index";
import { getProgress } from "@/lib/progress";
import TopicCard from "@/components/TopicCard";
import { BookOpen, Code2, FlaskConical, TrendingUp, ArrowRight, CheckCircle2, Trophy, Zap } from "lucide-react";

export default function LangHomeClient({ lang }: { lang: Language }) {
  const label = LANG_META[lang].label;
  const topics = getTopics(lang);
  const extraQuiz = getExtraQuiz(lang);
  const total = getTotalLessons(lang);
  const firstLessonHref = getFirstLessonHref(lang);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem(`${lang}_total_lessons`, String(total));
    }
  }, [lang, total]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="relative overflow-hidden rounded-3xl mb-10">
        ...
        <div className="absolute inset-0 opacity-10 select-none pointer-events-none hidden sm:block">
          <pre className="absolute top-6 right-6 text-[11px] leading-tight font-mono text-white whitespace-pre">
        ...
        <div className="relative px-8 sm:px-12 py-12 sm:py-16 text-white">
          ...
          <p className="text-blue-100 text-lg mb-8 max-w-2xl">
            Master {label} with bite-size lessons, hands-on coding practice,
      ...
      {/* Topics grid */}
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">{label} Topics</h2>
        <p className="text-gray-500">Master {label} step by step, from basics to advanced concepts.</p>
      </div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
        {topics.map((topic) => (
          <TopicCard key={topic.slug} topic={topic} language={lang} />
        ))}
      </div>

      {/* Featured row */}
      <div className="grid md:grid-cols-3 gap-4 mb-10">
        <Link href={`/${lang}/practice`} className="group bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-6 text-white hover:shadow-xl transition">
        ...
        <Link href={`/${lang}/quiz`} className="group bg-gradient-to-br from-purple-500 to-indigo-600 rounded-2xl p-6 text-white hover:shadow-xl transition">
        ...
        <Link href={firstLessonHref} className="group bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl p-6 text-white hover:shadow-xl transition">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl p-8 sm:p-10 text-center relative overflow-hidden">
        ...
        <div className="relative">
          <h2 className="text-2xl font-bold text-white mb-2">Ready to Master {label}?</h2>
